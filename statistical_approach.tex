...
received at periods $0, 1, \ldots, T$.  In the current specification, future
payoffs are discounted hyperbolically:
\begin{equation}
  \rho(t;\, r) = \frac{1}{1+r\,t}, \qquad r > 0,
  \label{eq:discount}
\end{equation}
...
by a domain-specific CARA utility function with loss aversion:
\begin{equation}
  u(x;\, \alpha_+,\, \alpha_-,\, \lambda,\, R) =
  \begin{cases}
    \dfrac{1-\exp[-\alpha_+(x-R)]}{\alpha_+}
      & \text{if } x \geq R, \\[10pt]
    -\lambda\,\dfrac{\exp[\alpha_-(R-x)]-1}{\alpha_-}
      & \text{if } x < R,
  \end{cases}
  \label{eq:utility}
\end{equation}
where $\alpha_+,\alpha_- \in (0, \infty)$ are gain- and loss-domain absolute
risk-aversion coefficients, and $\lambda > 0$ captures loss aversion when
$\lambda>1$.  The
\emph{present value} of an outcome stream is therefore
\begin{equation}
  \mathrm{PV}(\mathbf{o};\, r,\, R,\, \alpha_+,\, \alpha_-,\, \lambda)
  = u\!\left(\tilde{x}(\mathbf{o};\,r);\; \alpha_+,\, \alpha_-,\, \lambda,\, R\right).
  \label{eq:pv}
\end{equation}
The inverse of $u$ is used to convert CPT values back to the monetary scale:
\begin{equation}
  u^{-1}(y;\, \alpha_+,\, \alpha_-,\, \lambda,\, R) =
  \begin{cases}
    R - \dfrac{\ln(1-\alpha_+ y)}{\alpha_+}
      & \text{if } y \geq 0, \\[10pt]
    R - \dfrac{\ln(1-\alpha_- y/\lambda)}{\alpha_-}
      & \text{if } y < 0.
  \end{cases}
...
\begin{equation}
  ...
  = u^{-1}\!\bigl(V(L_\ell;\, R_\ell,\, \btheta);\;
                  \alpha_+,\, \alpha_-,\, \lambda,\, R_\ell\bigr)
  - Z_{t_\ell},
  \label{eq:ce_th}
\end{equation}
...
\begin{equation}
  \btheta = (r,\, \alpha_+,\, \alpha_-,\, \lambda,\,
             \gamma\text{ or }\alpha_p,\,
             a_1,\, a_2,\, a_3,\, \delta) \in \R^9.
  \label{eq:param_vec}
\end{equation}
...
$\btheta_k \in \R^9$ for $k = 1, \ldots, C$, and the mixing weights
...
The six structural CPT parameters
$(r,\, \alpha_+,\, \alpha_-,\, \lambda,\, \gamma\text{ or }\alpha_p,\, \delta)$
...
\begin{equation}
  \mu_{0,j} =
  \begin{cases}
    \ln(10^{-3}) & \text{for } \alpha_+,\, \alpha_-
                 \quad\text{(near risk-neutral CARA benchmark)}, \\[4pt]
    0 = \ln 1 & \text{for } \lambda,\, \gamma/\alpha_p,\, \delta
                 \quad\text{(no loss aversion / no distortion)}, \\[4pt]
...
Each cluster's six structural parameters inherit their log-location and
...
\begin{table}[h!]
  ...
  \begin{tabular}{lcc}
    ...
    $\alpha_+$    & $(10^{-5},\; 10^{-2})$ & $(10^{-5},\; 10^{-2})$ \\
    $\alpha_-$    & $(10^{-5},\; 10^{-2})$ & $(10^{-5},\; 10^{-2})$ \\
    $\lambda$     & $(0.5,\; 3.0)$       & $(0.5,\; 3.0)$     \\
    $\gamma$ / $\alpha_p$
                  & $(0.2,\; 1.0)$       & $(0.4,\; 1.0)$     \\
    $\delta$      & $1$ fixed            & $(0.0,\; 1.0)$     \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h!]
  ...
  \begin{tabular}{lll}
    ...
    $\mu_{0,\alpha}$      & $\ln(10^{-3})$ & Log-scale CARA benchmark for $\alpha_+,\alpha_-$ \\
    $\mu_{0,j}$           & $0.0$  & Log-scale benchmark for $\lambda,\gamma/\alpha_p,\delta$ \\
